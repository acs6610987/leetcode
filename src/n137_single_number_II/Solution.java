package n137_single_number_II;

//Simple solution
public class Solution {
    public int singleNumber(int[] nums) {
        int bitCount = 0, target = 0;
        for(int i = 0; i < 32; i++){
        	bitCount = 0;
	        for(int x : nums){
	        	if(((x>>i) & 1) == 1)
...
we just need to consider 1 bit. We know a number appears 3 times at most, so we need 2 bits to store that. Now we
have 4 state, 00, 01, 10 and 11, but we only need 3 of them.
